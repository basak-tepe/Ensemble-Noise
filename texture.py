...
water_smooth_cmap = LinearSegmentedColormap.from_list(
    "smooth_water",
    ["#1f4c7d", "#3a78a8", "#5bb4c5", "#88d1df"], 
    N=256
)
...
axs[0, 0].imshow(original_perlin, cmap="hot", origin="upper")
axs[0, 0].set_title("Perlin Noise")
axs[0, 0].axis("off")

axs[0, 1].imshow(original_random, cmap="copper", origin="upper")
axs[0, 1].set_title("Random Noise")
axs[0, 1].axis("off")

axs[0, 2].imshow(value_noise, cmap=wood_cmap, origin="upper")
axs[0, 2].set_title("Value Noise")
axs[0, 2].axis("off")

axs[0, 3].imshow(wavelet_noise, cmap="gist_earth", origin="upper")
axs[0, 3].set_title("Wavelet Noise")
axs[0, 3].axis("off")

axs[0, 4].imshow(plain_worley_noise, cmap= water_smooth_cmap, origin="upper")
axs[0, 4].set_title("Worley Noise")
axs[0, 4].axis("off")


axs[1, 0].imshow(combined_perlin, cmap="hot", origin="upper")
axs[1, 0].set_title("Lava (P. + P.)")
axs[1, 0].axis("off")

axs[1, 1].imshow(combined_perlin_random, cmap="copper", origin="upper")
axs[1, 1].set_title("Sand (P. + Random)")
axs[1, 1].axis("off")

axs[1, 2].imshow(perlin_value_combined, cmap=wood_cmap, origin="upper")
axs[1, 2].set_title("Wood (P. + Value)")
axs[1, 2].axis("off")

axs[1, 3].imshow(perlin_wavelet_combined, cmap="gist_earth", origin="upper")
axs[1, 3].set_title("Terrain (P. + Wavelet)")
axs[1, 3].axis("off")

axs[1, 4].imshow(worley_perlin_combined, cmap=water_smooth_cmap, origin="upper")
axs[1, 4].set_title("Water (P. + Worley)")
axs[1, 4].axis("off")
